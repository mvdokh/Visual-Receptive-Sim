...
- **Left panel** — View mode, 2D layer selector (when applicable), stimulus controls, **Circuit tuning** (**synaptic weights**, RGC population, spike output, **cell parameters**), and related options in the same scroll area.
- **Center** — The main viewport: 2D heatmap or 2D all-layers mosaic (and optionally other viewers depending on build).
- **Right panel (tabs)** — **Stats**, **Plots**, **Export**, **Recording** (session capture / playback).
...
.. list-table::
   ...
   * - **3D Stack**
     - (If enabled in your build) Signal-flow column, connectivity lines, per-slab traces.
...
- **Layer** combo selects which layer to display (Stimulus, Cones L/M/S, Horizontal, Bipolar ON, Amacrine, RGC Firing (L), RGC spikes (L), etc.).
...
Editable weights: Cone to Horizontal, Cone to Bipolar, Horizontal to Cone, Bipolar to Amacrine, Amacrine to Bipolar, Bipolar to RGC (signed range, about -3.0–3.0). They scale terms in the pipeline (see weight symbols in :doc:`/concepts/equations`, sections 2–7).

- **Reset weights to 1.0** — Restores defaults.
- **Randomize weights** — Uniform random in the same signed range for exploration.
...
- **Record** — While running, append frames (time, scalar summaries, downsampled ``fr_midget_on_L``, stimulus params without image pixels).
- **Save session to folder** — Writes ``session_meta.json`` and ``session.npz`` (see ``src/simulation/session_recording.py``).
- **Load session from folder** — Restores a recording for scrubbing.
- **Playback** — Pauses the live simulation; **Frame** slider restores stored stimulus parameters and upsampled RGC (L) map (not a full state replay).
